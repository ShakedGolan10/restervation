...
import { queryAllDocs } from '../../db/models/query_model.service';
import { Reservations } from '../../db/models/reservations';
import { ReservationsModel, SlotsModel } from '../../db/models/index.typegoose';
...
export async function addReservation(
  ...
  try {
    ...
    const newReservation = await addNewReservation(
      time,
      tableId,
      phone,
      slotId,
    ...
    await changeTableAvailability(tableId, slotId).catch((err) => {
...
export async function getReservationsByPhone(
  ...
  try {
    ...
    const queryResult = await queryAllDocs<typeof Reservations>(
      { reservedBy: phone },
      ReservationsModel,
    );
