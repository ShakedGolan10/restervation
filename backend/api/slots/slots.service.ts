...
import { queryAllDocs, queryById, QueryReturnType } from "../../models/query_model.service";
import { MongoSession, RestaurantModel, SlotsModel } from "../../models/index.typegoose";
import { buildEmptySlotsForDuration, getOpeningTime } from "../../service/util.service";
...
export async function getSlots(rest: QueryReturnType<typeof Restaurants>, date: string | Date) {
    try {
        const {openTime, closingTime} = getOpeningTime(new Date(date), rest.openingHours)
...
export async function changeTableAvailability(tableId: string, slotId: string) {
    try {
        const slotTables = (await queryById<typeof Slots>(slotId, SlotsModel)).tables
        const updatedTables = slotTables.map(table => {
            if (table.id === tableId) table.isAvailable = !table.isAvailable
            return table
        })
        await SlotsModel.findByIdAndUpdate(slotId, { $set: { tables: updatedTables } })
    } catch (error) {
