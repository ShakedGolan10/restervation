...
import { queryAllDocs, queryById } from '../../db/models/query_model.service';
...
export async function getRestaurants(req: GetRestaurant, res: Response) {
  try {
    console.log('req');
    const restaurants = await queryAllDocs<typeof Restaurants>(
      {},
      RestaurantModel,
      '-phone',
    );
...
export async function getRestaurant(req: GetRestaurant, res: Response) {
  try {
    const restaurant = await queryById<typeof Restaurants>(
      req.params.restId,
      RestaurantModel,
      'phone',
    );
